...
- Медиа. Каждая смена страницы вопроса сбрасывает состояние плеера на сервере в «играет с 0» (после пуша страницы), снимок
  `getGame` несёт `media` с позицией на момент запроса. ТВ и пульт запускают аудио/видео сами (`components/MediaPlayer.tsx`),
  пульт без звука по умолчанию и сообщает серверу только действия ведущего (расхождение больше 1 с или другое play/pause) —
  иначе будет эхо-петля. Chrome не даёт запускать звук до первого клика/клавиши на странице, а беззвучный автозапуск
  разрешает только `<video>`: ТВ показывает подсказку (`components/SoundUnlock.tsx`) и после нажатия догоняет пульт.
- «Следующий/Предыдущий раунд» на краях пака — `GameError`, кнопки в меню блокируются по `progress`.
- Windows-exe сервера: метаданные «SI Game» и иконку `scripts/package.js` ставит `resedit-cli` на копию базового Node из
  кэша pkg-fetch до упаковки (`PKG_NODE_PATH`); правка готового exe ломает приложенный payload. Запрос брандмауэра Windows
  называет свои правила по FileDescription («SI Game»), поэтому правило лаунчера называется «SI Game server».
