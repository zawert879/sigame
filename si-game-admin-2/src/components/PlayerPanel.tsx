...
export const PlayerPanel: FC<{ players: Player[] }> = ({ players }) => {
  return (<div className="h-[12vh] bg-gradient-to-b from-blue-700 to-blue-900 text-white border-b-2 border-b-white flex justify-around items-center">
    {players.map((player)=> <div key={player.id} className={`flex flex-col p-10 justify-center items-center ${player.queue === 0 ? 'bg-gradient-to-t from-first-color-from to-first-color-to' : ''} ${player.queue && player.queue > 0 ? 'bg-gradient-to-t from-second-color-from to-second-color-to' : ''} h-[12vh] grow`}>
