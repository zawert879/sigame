...
import { getRouteId } from "@/utils/route"
...
const Player: FC = () => {
  ...
  const gameId = getRouteId(router.query.id, 'player')
    ...
    <>
      {game && screen === Data.Screen.Initial && (<>
        <div className="bg-blue-700 h-[100vh] flex flex-col justify-center items-center w-screen text-yellow-200 text-9xl">
          <QRCode value={`${serverUrl}/admin/${gameId}`} color="white" size={600} />
          <Button onClick={() => { router.push(`/admin/${gameId}`) }}> GO </Button>
        </div>
      </>)}
      {game && screen !== Data.Screen.Initial &&
