...
- `/player/<id>` — экран игроков: табло, вопросы, медиа, звуки, итоги. На начальном экране — QR, кнопка GO
  для открытия пульта ведущего и кнопка «На весь экран»; на остальных экранах та же кнопка — значок в правом
  нижнем углу, появляется при движении мыши или фокусе (Tab, пульт) и прячется через несколько секунд.
- `/admin/<id>` — пульт ведущего: игроки и выбор пака, ход игры, очки, меню (раунды, повтор/отмена вопроса,
  настройки, выход).
- `/` и `/player` — открывают экран игроков первой игры на сервере, `/admin` — пульт ведущего первой игры.
